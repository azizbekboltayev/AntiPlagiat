...
def run_comparison_thread():
    global comparison_thread, pause_event, stop_event
...
def run_comparison(target_file, folder):
    results = compare_files(target_file, folder, progress_callback, pause_event, stop_event)
...
def stop_comparison():
    stop_event.set()
    if comparison_thread and comparison_thread.is_alive():
...
pause_button = ctk.CTkButton(frame, text="Pause", command=pause_comparison, fg_color="#FFC107", hover_color="#FFB300", font=("Helvetica", 16))
pause_button.grid(row=3, column=0, columnspan=1, padx=10, pady=10, sticky="ew")
pause_button.configure(state=tk.DISABLED)

stop_button = ctk.CTkButton(frame, text="Stop", command=stop_comparison, fg_color="#F44336", hover_color="#E53935", font=("Helvetica", 16))
stop_button.grid(row=3, column=1, columnspan=2, padx=10, pady=10, sticky="ew")
stop_button.configure(state=tk.DISABLED)
